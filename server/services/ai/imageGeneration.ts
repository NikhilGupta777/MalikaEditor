...
export async function generateAiImagesForVideo(
  ...
  aiLogger.debug(
    `Valid B-roll candidates: ${validCandidates.length}/${semanticAnalysis.brollWindows.length}`,
  );
  ...
  if (validCandidates.length <= maxImages) {
    ...
    for (let i = 0; i < maxImages; i++) {
      ...
      const segmentCandidates = validCandidates.filter(
      ...
      if (segmentCandidates.length > 0) {
        ...
        const best = segmentCandidates.sort(
          (a, b) =>
            (priorityOrder[a.priority] ?? 1) - (priorityOrder[b.priority] ?? 1),
        )[0];
        aiImageCandidates.push(best);
      } else {
        const unusedCandidates = validCandidates.filter(
          (c) =>
            !aiImageCandidates.includes(c) &&
            Math.abs(c.start - (segmentStart + segmentDuration / 2)) <
              segmentDuration,
        );
  ...
  aiLogger.debug(
    `AI Image candidates after selection: ${aiImageCandidates.length} (targeting ${maxImages})`,
  );
