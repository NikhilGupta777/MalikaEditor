...
function calculateRelevanceScore(
  ...
  // Apply true temporal decay (Half-life of 30 days)
  const decayFactor = Math.pow(0.5, ageInDays / 30);
  score *= (0.7 + 0.3 * decayFactor); // Decay at most 30% of the score over time
...
export async function storePattern(
  ...
  if (reviewData.editPlan?.actions) {
    for (const action of reviewData.editPlan.actions) {
      ...
      // Store all patterns, including negative ones (success < 50)
